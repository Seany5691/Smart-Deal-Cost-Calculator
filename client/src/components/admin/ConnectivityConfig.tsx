...
interface ConnectivityConfigProps {
  onOfflineChange: (data: Item[]) => void;
}

const ConnectivityConfig = ({ onOfflineChange }: ConnectivityConfigProps) => {
  ...
  const { sections, updateConnectivity } = useCalculatorStore();
  ...
  useEffect(() => {
    ...
    if (connectivitySection) {
      setItems(connectivitySection.items);
    }
  }, [sections]);
  ...
  const saveItems = async (updatedItems: Item[]) => {
    try {
      const success = await updateConnectivity(updatedItems);
      ...
      if (success) {
        ...
        setToast({
          show: true,
          title: 'Success',
          message: 'Connectivity items updated successfully',
          type: 'success'
        });
      } else {
        setToast({
          show: true,
          title: 'Error',
          message: 'Failed to update connectivity items',
          type: 'error'
        });
      }
    } catch (error) {
      console.error('Error saving connectivity items:', error);
      setToast({
        show: true,
        title: 'Error',
    ...
    if (selectedItem) {
      ...
      updatedItems = items.map(item => 
        item.id === selectedItem.id 
          ? { ...item, name: itemName, cost: itemCost, locked: itemLocked }
          : item
      );
    } else {
      // Add new item
              ...
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-lg font-medium text-gray-900">
                    {selectedItem ? 'Edit Connectivity Item' : 'Add Connectivity Item'}
                  </h3>
                  <button
                    onClick={onClose}
