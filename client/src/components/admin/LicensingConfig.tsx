...
const LicensingConfig = ({ onOfflineChange }: LicensingConfigProps) => {
  ...
  const saveItems = async (updatedItems: Item[]) => {
    try {
      const success = await updateLicensing(updatedItems);
      ...
      if (success) {
        ...
        setToast({
          show: true,
          title: 'Success',
          message: 'Licensing items updated successfully',
          type: 'success'
        });
      } else {
        setToast({
          show: true,
          title: 'Error',
          message: 'Failed to update licensing items',
          type: 'error'
        });
      }
    } catch (error) {
      console.error('Error saving licensing items:', error);
      setToast({
        show: true,
        title: 'Error',
              ...
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-lg font-medium text-gray-900">
                    {selectedItem ? 'Edit Licensing Item' : 'Add Licensing Item'}
                  </h3>
                  <button
                    onClick={onClose}
