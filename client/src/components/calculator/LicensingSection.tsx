...
const LicensingSection = ({ onNext, onPrev }: LicensingSectionProps) => {
  ...
  useEffect(() => {
    const total = licensingItems.reduce(
  ...
  const handleQuantityChange = (itemId: string, quantity: number) => {
    updateSectionItem('licensing', itemId, { quantity });
  };
  ...
  const handleAddTemporaryItem = () => {
    ...
    updateSectionItem('licensing', tempItem.id, tempItem);
    ...
    setToast({
      show: true,
      title: 'Success',
      message: `Added ${tempItem.name} to licensing items`,
      type: 'success'
    });
  };
  ...
  const handleSave = () => {
    ...
    setToast({
      show: true,
      title: 'Success',
      message: `Licensing selections saved. Monthly licensing cost: ${formatCurrency(totals.licensingCost)}`,
      type: 'success'
    });
    ...
    <div className="p-4">
      ...
      <div className="flex justify-between items-center mb-4">
        <p className="font-bold">
          Total Monthly Licensing Cost: {formatCurrency(totalMonthlyCost)}
        </p>
      </div>

      {/* Add Temporary Item Form */}
      <div className="bg-gray-50 p-4 rounded-md mb-6 border border-gray-200">
        <h3 className="text-md font-semibold mb-3">Add Temporary Licensing Item</h3>
        <p className="text-sm text-gray-600 mb-4">Add a custom licensing item for this calculation only. This item will not be saved for future calculations.</p>
