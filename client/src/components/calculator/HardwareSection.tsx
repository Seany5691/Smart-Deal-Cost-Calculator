...
const HardwareSection = ({ onNext, onPrev }: HardwareSectionProps) => {
  ...
  const handleQuantityChange = (itemId: string, quantity: number) => {
    updateSectionItem('hardware', itemId, { quantity });
  };
  ...
  const handleAddTemporaryItem = () => {
    ...
    updateSectionItem('hardware', tempItem.id, tempItem);
    ...
    setToast({
      show: true,
      title: 'Success',
      message: `Added ${tempItem.name} to hardware items`,
      type: 'success'
    });
  };
  ...
  const handleSave = () => {
    ...
    setToast({
      show: true,
      title: 'Success',
      message: `Hardware selections saved. Total hardware cost: ${formatCurrency(totals.hardwareTotal)}`,
      type: 'success'
    });
    ...
    <div className="p-4">
      ...
      <div className="flex justify-between items-center mb-4">
        <p className="font-bold">
          Total Hardware Cost: {formatCurrency(totalHardwareCost)}
        </p>
      </div>

      {/* Add Temporary Item Form */}
      <div className="bg-gray-50 p-4 rounded-md mb-6 border border-gray-200">
        <h3 className="text-md font-semibold mb-3">Add Temporary Hardware Item</h3>
        <p className="text-sm text-gray-600 mb-4">Add a custom hardware item for this calculation only. This item will not be saved for future calculations.</p>
